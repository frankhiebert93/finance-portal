...
export default async function PersonalDashboard({ searchParams }: { searchParams: any }) {
    ...
    // Liquid Vault & Carry Over Engine
    let carryOverSurplus = 0
    let bankBalance = 0
    let cashBalance = 0

    // Master function to calculate where the money is
    const processWallet = (tx: any) => {
        const catData = Array.isArray(tx.categories) ? tx.categories[0] : tx.categories;
        const amt = Number(tx.amount || 0);
        const wallet = tx.wallet || 'bank'; // Defaults old transactions to bank

        if (!catData) {
            // It's a Transfer or Uncategorized (Amount is already positive/negative from the action)
            if (wallet === 'bank') bankBalance += amt;
            else if (wallet === 'cash') cashBalance += amt;
            return amt; // Returns amount for the surplus total (perfectly cancels out globally)
    ...
    // Run prior history through the vault engine
    allPriorHistory?.forEach((tx: any) => {
        carryOverSurplus += processWallet(tx);
    })
    ...
    transactions?.forEach((tx: any) => {
        // First, add it to the physical wallet vaults
        processWallet(tx);

        // Then process for monthly budget charts (Ignoring transfers)
        const catData = Array.isArray(tx.categories) ? tx.categories[0] : tx.categories;
        if (catData) {
            const amt = Number(tx.amount || 0)
    ...
    // Generate 6-Month Trend Data (Ignoring transfers)
    ...
    return (
        <div className="bg-gradient-to-br from-indigo-50 via-white to-purple-50 font-sans min-h-screen pb-20">
            <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6 md:space-y-8">
                ...
                {/* Liquid Vaults - Cash vs Bank */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-indigo-500 to-indigo-700 p-5 rounded-2xl shadow-lg text-white">
                        <h3 className="text-[10px] font-black text-indigo-200 uppercase tracking-widest">🏦 Bank Vault</h3>
                        <p className="text-3xl font-black mt-1">${fmt(bankBalance)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-emerald-500 to-emerald-700 p-5 rounded-2xl shadow-lg text-white">
                        <h3 className="text-[10px] font-black text-emerald-200 uppercase tracking-widest">💵 Cash on Hand</h3>
                        <p className="text-3xl font-black mt-1">${fmt(cashBalance)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-5 rounded-2xl shadow-lg text-white">
                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">💰 Total Liquidity</h3>
                        <p className="text-3xl font-black mt-1">${fmt(bankBalance + cashBalance)}</p>
                    </div>
                </div>

                {/* Top Level Metrics for the Selected Month */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                        <h3 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Remaining To Allocate</h3>
                        <p className="text-2xl font-black text-slate-900">${fmt(remainingToAllocate)}</p>
                    </div>
                </div>
